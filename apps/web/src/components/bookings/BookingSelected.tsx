...
const BookingSelected = ({
  ...
  const existingReview = alreadyReviewed.find(
    (review) =>
      review.technician.id === selectedBooking.technician.id &&
      review.user.id === selectedBooking.user.id,
  );
    ...
    <Dialog open={!!selectedBooking} onOpenChange={closeModal}>
      <DialogContent className="bg-gradient-to-br from-zinc-900 to-zinc-800 border-zinc-700/60 max-w-2xl max-h-[90vh] overflow-y-auto text-white">
        <DialogHeader>
          ...
          <DialogDescription className="text-zinc-400">
            ...
            {isTechnician
              ...
              : selectedBooking.technician?.user?.firstName &&
                  selectedBooking.technician?.user?.lastName
                ? `${selectedBooking.technician.user.firstName} ${selectedBooking.technician.user.lastName}`
                : selectedBooking.technician?.user?.username ||
                  "Técnico sin nombre"}
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-4">
          ...
          <div>
            <h3 className="font-semibold text-white mb-2">Comentario</h3>
            <div className="bg-gray-50 p-3 rounded">
              <p className="text-black">{selectedBooking.comment}</p>
            </div>
          </div>
                ...
                <div className="space-y-3">
                  ...
                  <div className="flex items-start gap-2 bg-blue-900/30 border border-blue-800/50 p-3 rounded-lg">
                    ...
                    <p className="text-sm text-blue-300">
                      Ya has escrito una reseña para este técnico. Las reseñas
                      ayudan a otros usuarios a tomar mejores decisiones.
                    </p>
                  </div>
                </div>
              ) : (
        ...
        <div className="flex justify-end gap-2 mt-6">
          ...
          {(selectedBooking.status === "Aceptado" ||
            selectedBooking.status === "Completado") && (
            <WhatsAppButton
              phoneNumber={
                isTechnician
                  ? selectedBooking.user?.phone || ""
          ...
          <DialogClose asChild>
            <Button className="bg-red-500 text-white hover:bg-red-700 hover:text-white">
              Cerrar
            </Button>
          </DialogClose>
        </div>
