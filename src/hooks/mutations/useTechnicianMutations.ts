...
export const useUpdateUser = () => {
    ...
    return useMutation({
        mutationFn: async (techData: Technicians) => {
            if (!user?.id) throw new Error('Usuario no autenticado');
            logger.debug('Actualizando perfil de usuario', { userId: user.id, techData });
            return await updateUserDataRequest(user.id, techData);
        },
        onSuccess: (data) => {
